...
typedef struct {
    ...
    PrimitiveCoord position;
    MapPropriety *aditionalInfo;
...
} PrimitiveMapStructure;

typedef struct {
    
    const char *path;
    simple_vector entitys;
    simple_vector structures;
    simple_vector aditionalInfo;
...
simple_vector load_primitive_map_tructures(FILE *file){
    simple_vector ret;
    return ret;
}
...
PrimitiveMap* load_primitive_map(const char *path){
    ...
    ret->structures = load_primitive_map_tructures(file);
...
void print_primitive_map_content(PrimitiveMap *map){
    ...
    printf("    map additional info size: %d\n",map->aditionalInfo.size);
    printf("    map additional info data: {\n");
